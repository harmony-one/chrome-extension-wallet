        ...
        <main class="main">
            ...
            <div v-else>
                <div class="token" v-for="token in account.tokens" :key="token.name" v-show="token.name !== '_'">
                    ...
                    <span class="token-balance">{{ $formatNumber(getTokenAmount(token.balance,  getHRC20Details(token.name)[2]), { maximumSignificantDigits: parseInt(getHRC20Details(token.name)[2]) + 1 }) }}</span>
                </div>
            </div>
        </main>
    </div>
