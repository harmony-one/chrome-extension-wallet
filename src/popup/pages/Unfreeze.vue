<template>
  <div>
    ...
    <main class="main">
      ...
      >
        <div class="form-info">
          Frozen tokens are "locked" for a period of 3 days. During this period
          the frozen TRX cannot be traded. After this period you can unfreeze
          the TRX and trade the tokens.
        </div>
    ...
    <confirm-dialog
      text="Are you sure you want to unfreeze TRX ?"
      ref="confirmDialog"
      @confirmed="unfreezeBalance"
    />
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    frozenExpires() {
      return (
        this.$formatDate(this.account.frozenExpires) +
        " " +
        this.$formatTime(this.account.frozenExpires)
      );
    },
    ...mapState({
  ...
  methods: {
    async unfreezeBalance() {
      ...
      try {
        const { success } = await API().unfreezeBalance(this.wallet.address)(
          wallet.privateKey
        );
