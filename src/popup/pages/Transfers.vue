<template>
    <div>
        ...
        <main class="main">
            ...
            <div v-else>
                <div>
                    <external-link :url="getTransferLink(transfer.hash)" class="transfer" v-for="transfer in transfers" :key="transfer.hash">
                ...
                <a class="load-more" v-show="transfers.length < txCount && ! loadMoreLoading" href="#" @click="loadMore">Load More</a>
            </div>
        </main>
    </div>
</template>
...
<script>
    ...
    import { getTransfers, getNetworkLink, getTransactionCount } from '../../lib/keystore'
    ...
    export default {
        ...
        methods: {
            async loadTransfers() {
                ...
                this.page = 0
                const transfersData = await getTransfers(this.address, this.page, this.limit)
            ...
            formatTokenAmount(transfer) {
                return new Unit(transfer.value).asWei().toEther().toString()
            },
            ...
            formatShard(transfer) {
                return 'shard from ' + transfer.shardID.toString() + ' to ' + transfer.toShardID.toString()
            }
        }
    }
</script>
